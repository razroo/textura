...
<p style="margin-top:10px"><strong>Why this is novel for small models.</strong> Current world-model methods fail here: <em>vision-based</em> approaches (Genie, SORA) require large multimodal models that small text-only models can't use. <em>Browser/DOM-based</em> data generation runs at ~2 layouts/sec — too slow for the millions of examples small models need. <em>Fixed metadata</em> requires manual annotation and doesn't scale. <em>Game engines</em> output pixels, not tokens.</p>
<p>Textura's approach is different: the geometry <strong>emerges from the text content itself</strong>. "EXIT" produces a small bounding box. "DO NOT ENTER — restricted area. Authorized personnel only beyond this point" produces a large one. The model learns <em>language grounding through geometry</em> — longer, more descriptive text = larger spatial footprint. The pipeline: LLM generates room descriptions (infinite variation, zero annotation) → Textura computes accurate layout in &lt;1ms → serialize as tokens → train. No other method gives you all three: <strong>text-native output</strong> (small models can consume it), <strong>content-derived geometry</strong> (no manual annotation), and <strong>speed</strong> (millions of examples per hour on one thread).</p>`,
}
